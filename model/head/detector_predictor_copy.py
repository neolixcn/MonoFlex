...
class _predictor_test(nn.Module):
    def __init__(self, cfg, in_channels):
        super(_predictor_test, self).__init__()
        ...
        if self.enable_edge_fusion:
            ...
            edge_indices = self.get_edge_utils((self.input_width, self.input_height), np.array([0, 0])).numpy()
            input_edge_indices[:edge_indices.shape[0]] = edge_indices
            if cfg.DATASETS.TEST_SPLIT == 'test':
                self.input_edge_count =  torch.from_numpy(np.array([edge_indices.shape[0] -1]))
                self.edge_indices =  torch.from_numpy(input_edge_indices )
                self.grid_edge_indices = self.edge_indices.view(1, -1, 1, 2).float()
                self.grid_edge_indices[..., 0] = self.grid_edge_indices[..., 0] / (self.output_width - 1) * 2 - 1
                self.grid_edge_indices[..., 1] = self.grid_edge_indices[..., 1] / (self.output_height - 1) * 2 - 1
            else:
                self.input_edge_count =  torch.from_numpy(np.array([edge_indices.shape[0] -1])).cuda()
                self.edge_indices =  torch.from_numpy(input_edge_indices ).cuda()
                self.grid_edge_indices = self.edge_indices.view(1, -1, 1, 2).float()
                self.grid_edge_indices[..., 0] = self.grid_edge_indices[..., 0] / (self.output_width - 1) * 2 - 1
                self.grid_edge_indices[..., 1] = self.grid_edge_indices[..., 1] / (self.output_height - 1) * 2 - 1
        ...
        for i, reg_feature_head in enumerate(self.reg_features):
            ...
            for j, reg_output_head in enumerate(self.reg_heads[i]):
                ...
                if self.enable_edge_fusion and i == self.offset_index[0] and j == self.offset_index[1]:
                    edge_indices = self.edge_indices#torch.stack([t.get_field("edge_indices") for t in self.targets]) # B x K x 2
                    edge_lens =  self.input_edge_count#torch.stack([t.get_field("edge_len") for t in self.targets]) # B
                
                    # normalize
                    
                    # grid_edge_indices=grid_edge_indices.to(feature_cls.get_device())
                    # apply edge fusion for both offset and heatmap
                    feature_for_fusion = torch.cat((feature_cls, reg_feature), dim=1)
                    edge_features = F.grid_sample(feature_for_fusion, self.grid_edge_indices, align_corners=True).squeeze(-1)
                    ...
                    edge_indices = edge_indices.view(b, -1, 2)
                    for k in range(b):
